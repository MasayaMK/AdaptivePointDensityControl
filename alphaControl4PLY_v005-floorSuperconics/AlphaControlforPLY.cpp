...
inline double circle(double x, double y, double z){
    ...
    double e2 = 0.1;
    double a = 20.20364;//x軸の半径
    double b = 6.9848;//y軸の半径
    double c = 4.7885;//z軸の半径
    
    double A = pow((x-(-14.75766))/a, 2/e1);//14.75766
    double B = pow((y-10.8181)/b, 2/e1);//10.8181
    double C = pow((z-2.4959)/c, 2/e2);//2.4959
...
void AlphaControlforPLY::setParticles( kvs::PolygonObject* ply)
{
  ...
  for( size_t i=0; i<numVert; i++ ) {
    ...
    //超楕円式の関数値を取得
    double tt = pow(1-circle(x,y,z), 5);
