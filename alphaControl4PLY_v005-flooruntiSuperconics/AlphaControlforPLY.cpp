...
//楕円式
inline double circle(double x, double y, double z){
    double e1 = 0.1;//楕円の形を決定
    double e2 = 0.1;
    double a = 19.00364;//x軸の半径(18.50364)
    double b = 6.4848;//y軸の半径
    double c = 5.7885;//z軸の半径
    
    double A = pow((x-(-14.75766))/a, 2/e1);
    double B = pow((y-10.8181)/b, 2/e1);
    double C = pow((z-2.4959)/c, 2/e2);
    return (A+B+C);//この計算結果が１に近ければ外側、０に近ければ中心側
}
...
void AlphaControlforPLY::setParticles( kvs::PolygonObject* ply)
{
  ...
  for( size_t i=0; i<numVert; i++ ) {
    ...
    kvs::Vector3f pc(-15.20766, 11.6181, 2.4959);//バウンディングボックス値の中心(平均)をベクトルに格納(akarenga0.025_floor.ply)
    ...
    double tt = pow(circle(x,y,z), 5);//点が楕円の内側か外側かを関数値で判断している
    if(tt>1.0)
        tt=1.0;
    double t = (m_ratio*tt);//関数値を考慮して最終的な不透明度を決定
    int n = floor(t);//不透明度の整数部分だけを代入
    double a = t - n;//不透明度の小数部分だけを代入
    ...
    int j;//ループカウンタ
      ...
      double R = uniRand();//乱数を代入
      
      //ランダム関数の値を元に点を描くか描かないかを決定
      if(a > R){
          SetCoords.push_back( x );
          SetCoords.push_back( y );
          SetCoords.push_back( z );
